...
public class NavigationFactory : NSObject, FlutterStreamHandler
{
    ...
    func startNavigation(routeResponse: RouteResponse, options: NavigationRouteOptions, navOptions: NavigationOptions)
    {
        ...
        {
            // Apply UIAppearance proxies before creating NavigationViewController
            // so views are styled correctly at creation time (especially Steps list labels)
            let isDarkForStyle = _isDarkTheme || (_mapStyleUrlDay?.lowercased().contains("dark") == true)
            if isDarkForStyle {
                CustomNightStyle().apply()
            } else {
